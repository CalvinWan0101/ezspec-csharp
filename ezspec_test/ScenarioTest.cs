...
public class ScenarioTest {
    ...
    public void all_steps_use_the_same_scenario_environment() {
        ...
        scenario
            ...
            .Then("I get the answer equre to ${90}.", env => { Assert.AreEqual(env.GetIntArg(0), env.GetInt("average")); })
            .Execute();
    }
    ...
    public void step_terminate_after_failure() {
        ...
        Assert.ThrowsException<EzSpecError>(() => {
            scenario
                ...
                .When("I run when step and failed", Step.TerminateAfterFailure, env => { Assert.AreEqual(1, 2); })
    ...
    public void step_continue_after_failure() {
        ...
        Assert.ThrowsException<EzSpecError>(() => {
            scenario
                .Given("a given step.", env => { })
                .When("I run when step and failed", Step.ContinuousAfterFailure, env => { Assert.AreEqual(1, 2); })
    ...
    public void step_pending() {
        ...
        scenario
            .Given("a given step.", env => { })
            .When("I run when step and failed", env => { PendingException.Pending("the step not done"); })
            .Then("this step should not be skip", env => { })
            .Execute();
    ...
    public void step_execute_with_background_failure() {
        ...
        Assert.ThrowsException<EzSpecError>(() => {
            rule.NewBackground("Background's name")
                .Given("a failure step", env => { Assert.IsTrue(false); });
    ...
    public void scenario_to_string() {
        ...
        Assert.ThrowsException<EzSpecError>(() => {
            scenario
                .Given("a given step", env => { })
                .When("I run when step", env => { Assert.IsTrue(true); })
                .Then("this step failure", Step.ContinuousAfterFailure, env => { Assert.IsTrue(false); })
                .And("this step success", env => { Assert.IsTrue(true); })
                .But("nothing", env => { })
                .Execute();
        });
    ...
    public void scenario_with_step_contain_more_than_one_line_description() {
        ...
        Assert.ThrowsException<EzSpecError>(() => {
            scenario
                ...
                .When("I run when step line 1\nI run when step line 2", env => { Assert.IsTrue(true); })
                .Then("this step failure line 1\nthis step failure line 2", Step.ContinuousAfterFailure, env => { Assert.IsTrue(false); })
                .And("this step success line 1\nthis step success line 2", env => { Assert.IsTrue(true); })
                .But("nothing line 1\nnothing line 2", env => { })
                .Execute();
        });
    ...
    public void scenario_with_step_contain_long_string_description() {
        ...
        Assert.ThrowsException<EzSpecError>(() => {
            scenario
                ...
                .When(@"I run when step line 1
                            I run when step line 2".AutoTrim(), env => { Assert.IsTrue(true); })
                .Then(@"this step failure line 1
                            this step failure line 2".AutoTrim(), Step.ContinuousAfterFailure, env => { Assert.IsTrue(false); })
                .And(@"this step success line 1
                            this step success line 2".AutoTrim(), env => { Assert.IsTrue(true); })
                .But(@"nothing line 1
